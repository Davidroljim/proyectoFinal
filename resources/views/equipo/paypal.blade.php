...
<div >
    <div class="container">
        ...
        <div style="text-align: center">
            ...
            <h1 style="text-align: center">Los artículos que tenías en el carrito ya no están en stock</h1>
        ...
        background-color: #1b7cb8;">Seguir Alquilando</a>
         ...
         @php
            ...
            @endif
            <h1 class="mb-5">Pagar con PayPal</h1>
            ...
            <div class="contenedorProducto">
                ...
                <div>
                    ...
                    @foreach($carritos as $cart)
                        <div  style="display:flex;">
                            <div style="width: 10%;min-width: 95px;margin-right: 18px;">
                                <a href="">
                                    <img style="width: 100%;" src="{{$cart->equipo->fotos}}">
                                </a>

                            </div>
                            <div style="width: 100%;">
                                <div style="display: flex;justify-content: space-between;width: 100%">
                                    <a href="" style="font-size:15px;text-decoration: none;color: black"><p><b>{{$cart->equipo->nombre}}</b></p></a>


                                      <form action="{{ route('carrito.destroy',$cart->id) }}" method="POST">
                                      @csrf
                                      @method('DELETE')
                                      <input type="hidden" name="borrarCarrito" value="borrarCarrito">
                                      <button type="submit" style="border-radius: 8px; background-color: #eee;border:none;color: red;font-weight: 700;text-decoration: none;font-size: 20px"><i class="fa fa-fw fa-trash"></i> {{ __('X') }}</button>
                                    </form>
                                </div>

                                <div style="display: flex;">
                                    <p style="margin-right: 5%">Fecha Inicio alquiler = {{$cart->f_inicio}}</p>
                                    <p>Fecha Fin alquiler = {{$cart->f_fin}}</p>
                                </div>
                                @php
                                
                                if ($precioFin == null) {
                                    $fi = new DateTime($cart->f_inicio);
                                $ff = new DateTime($cart->f_fin);
                                $diff = $fi->diff($ff); 
            
                                $diasAlquiler=$diff->days+1;
                                echo ($diasAlquiler);
                                echo ($cart->precio);
                                $precioFin=($cart->precio)*$diasAlquiler;
                                }else{
                                    $fi = new DateTime($cart->f_inicio);
                                $ff = new DateTime($cart->f_fin);
                                $diff = $fi->diff($ff); 
                                ...
                                $diasAlquiler=$diff->days+1;
                                $precioFin=$precioFin+($cart->precio)*$diasAlquiler; 
                                }
                                
                                @endphp
                                 <p id="precios" data-price={{($cart->precio)*$diasAlquiler}} class="precios" >{{($cart->precio)*$diasAlquiler}}€</p>
                            </div>
                    ...
                    <a href="{{route('catalogo')}}" class="btn  " style=" font-size: 21px;margin-bottom: 15px; color: white;
            background-color: #1b7cb8;">Seguir Alquilando</a>
                    </div>
                </div>

                <div class="" style="text-aling: center; width: 100%;">
                    
                   
                        <div class="precioTotal" style="text-aling: center; display: flex;justify-content: space-between;width: 25%;">
                            <p style="font-size: 20px"><strong>Total:</strong></p>
                            <p style="font-size: 20px" id="precioTotal">{{$precioFin}}€</p>
...
<script src="https://www.paypal.com/sdk/js?client-id=AYd0gYeyYQ_4pD0O8XJIlxoG9r1ldEx7uD4pGvlYuoVc4EOft8aCDJfHSzwN6Pucg1jJC7-gudKK1dsR&currency=EUR&locale=de_DE"></script>
    <script>
        paypal.Buttons({
        ...
        return actions.order.create({
            ...
            onApprove: function(data, actions) {
                return actions.order.capture().then(function(details) {
                    // Aquí puedes mostrar un mensaje de confirmación o redirigir a otra página.
                    console.log('Pago capturado: ' + details.id);
                    var url = "{{ route('carritos.pago') }}";
